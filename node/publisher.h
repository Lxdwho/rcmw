...
#ifndef _RCMW_NODE_PUBLISHER_H_
...
void Publisher<MessageT>::GetSubscribers(std::vector<RoleAttributes>* subscribers) {
    if(subscribers == nullptr) return;
    if(!PublisherBase::IsInit()) return;
...
void Publisher<MessageT>::OnChannelChange(const ChangeMsg& change_msg) {
    if(change_msg.role_type != RoleType::ROLE_READER) return ;

    auto subscriber_attr = change_msg.role_attr;
    if(subscriber_attr.channel_name != this->role_attr_.channel_name) return;
