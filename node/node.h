...
#ifndef _RCMW_NODE_NODE_H_
#define _RCMW_NODE_NODE_H_
...
public:
    ...
    auto CreatePublisher(const RoleAttributes& role_attr) -> std::shared_ptr<Publisher<MessageT>>;

    template <typename MessageT>
    auto CreatePublisher(const std::string& channel_name) -> std::shared_ptr<Publisher<MessageT>>;
...
template <typename MessageT>
auto Node::CreatePublisher(const RoleAttributes& role_attr) -> std::shared_ptr<Publisher<MessageT>> {
    return node_channel_impl_->template CreatePublisher<MessageT>(role_attr);
}

template <typename MessageT>
auto Node::CreatePublisher(const std::string& channel_name) -> std::shared_ptr<Publisher<MessageT>> {
...
auto Node::CreateSubscriber(const std::string& channel_name, 
    ...
    if(subscribers_.find(channel_name) != subscribers_.end()) {
        AWARN << "Failed to create subscriber: subscriber with the same channel already exists.";
        return nullptr;
    }

    auto subscriber = node_channel_impl_->template CreateSubscriber<MessageT>(channel_name, reader_func);
    if(subscriber != nullptr) subscribers_.emplace(std::make_pair(channel_name, subscriber));
    return subscriber;
}
...
auto Node::CreateSubscriber(const SubscriberConfig& config,
    ...
    auto subscriber = node_channel_impl_->template CreateSubscriber<MessageT>(config, reader_func);
    if(subscriber != nullptr) subscribers_.emplace(std::make_pair(config.channel_name, subscriber));
    return subscriber;
}
...
auto Node::CreateSubscriber(const RoleAttributes& role_attr, 
    ...
    auto subscriber = node_channel_impl_->template CreateSubscriber<MessageT>(role_attr, reader_func);
    if(subscriber != nullptr) subscribers_.emplace(std::make_pair(role_attr.channel_name, subscriber));
    return subscriber;
}
...
auto Node::GetSubscriber(const std::string& channel_name)
    ...
    if(it != subscribers_.end()) return std::dynamic_pointer_cast<Subscriber<MessageT>>(it->second);
    return nullptr;
}
