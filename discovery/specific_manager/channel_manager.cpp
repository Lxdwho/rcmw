...
void ChannelManager::GetUpstreamOfNode(const std::string& node_name, 
    ...
    for(auto& reader : readers) channels.emplace(reader.channel_name);

    RoleAttrVec writers;
    for(auto& channel : channels) GetWritersOfChannel(channel, &writers);
    ...
    for(auto& item : nodes) upstream_nodes->emplace_back(item.second);
}
...
void ChannelManager::GetDownStreamOfnode(const std::string& node_name, 
    ...
    RoleAttrVec writers;
    GetReadersOfNode(node_name, &writers);
    ...
    for(auto& item : nodes) downstream_nodes->emplace_back(item.second);
}
...
void ChannelManager::GetMsgType(const std::string& channel_name, 
    ...
    if(!channel_writers_.Search(key, &writer)) {
        AINFO << "can not find writer of channel: " << channel_name 
              << " key: " << key;
        return;
    }
...
void ChannelManager::ScanMessageType(const ChangeMsg& msg) {
    ...
    for(auto& w_attr : existed_writers) {
        if(!IsMessageTypeMatching(w_attr.message_type, msg.role_attr.message_type)) {
            AINFO << "newly added " << role_type << "(belongs to node["
