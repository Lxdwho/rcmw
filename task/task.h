...
static auto Async(F&& f, Args&&... args) -> std::future<typename std::result_of<F(Args...)>::type> {
    return TaskManager::Instance()->Enqueue(std::forward<F>(f), std::forward<Args(args)...);
}
...
static void SleepFor(const std::chrono::duration<Rep, Period>& sleep_duration) {
    ...
    if(routine == nullptr) std::this_thread::sleep_for(sleep_duration);
    else routine->Sleep(sleep_duration);
}

static inline void USleep(useconds_t usec) {
    auto routine = croutine::CRoutine::GetCurrentRoutine();
    if(routine == nullptr) std::this_thread::sleep_for(std::chrono::microseconds{usec});
    else routine->Sleep(croutine::Duration(usec));
}
