...
DataStream::ByteOrder DataStream::byteorder() {
    ...
    if(str[0] == 0x12) return ByteOrder::BigEndian;
    else return ByteOrder::LittleEndian;
}
...
bool DataStream::read(bool& value) {
    if(m_buf[m_pos] != DataType::BOOL) return false;
    ++m_pos;
    value = m_buf[m_pos];
    ++m_pos;
    return true;
...
bool DataStream::read(char& value) {
    if(m_buf[m_pos] != DataType::CHAR) return false;
    ++m_pos;
    value = m_buf[m_pos];
    ++m_pos;
    return true;
...
bool DataStream::read(int32_t& value) {
    if(m_buf[m_pos] != DataType::INT32) return false;
    ++m_pos;
...
bool DataStream::read(uint32_t& value) {
    if(m_buf[m_pos] != DataType::UINT32) return false;
    ++m_pos;
...
bool DataStream::read(uint64_t& value) {
    if(m_buf[m_pos] != DataType::UINT64) return false;
    ++m_pos;
...
bool DataStream::read(int64_t& value) {
    if(m_buf[m_pos] != DataType::INT64) return false;
    ++m_pos;
...
bool DataStream::read(float& value) {
    if(m_buf[m_pos] != DataType::FLOAT) return false;
    ++m_pos;
...
bool DataStream::read(double& value) {
    if(m_buf[m_pos] != DataType::DOUBLE) return false;
    ++m_pos;
...
bool DataStream::read(std::string& value) {
    if(m_buf[m_pos] != DataType::STRING) return false;
    ++m_pos;
    int len;
    read(len);
