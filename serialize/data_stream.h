...
public:
    ...
    DataStream();
    DataStream(const std::string& str);
...
bool DataStream::read(std::vector<T>& value) {
    value.clear();
    if(m_buf[m_pos] != DataType::VECTOR) return false;
    ++m_pos;
    int len;
    read(len);
    value.resize(len);
    ...
    T* ptr = value.data();
    char* cha_ptr = reinterpret_cast<char*>(ptr);
    read(cha_ptr, len);
    return true;
}
...
bool DataStream::read(std::list<T>& value) {
    value.clear();
    if(m_buf[m_pos] != DataType::LIST) return false;
    ++m_pos;
    int len;
    read(len);
...
bool DataStream::read(std::map<K, V>& value) {
    value.clear();
    if(m_buf[m_pos] != DataType::MAP) return false;
    ++m_pos;
    int len;
    read(len);
...
bool DataStream::read(std::set<T>& value) {
    value.clear();
    if(m_buf[m_pos] != DataType::SET) return false;
    ++m_pos;
    int len;
    read(len);
